...
function App() {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      for (const edge of existing) if (!selected.includes(edge.upstream_task_id)) await request(`/dependencies/${edge.id}`, { method: 'DELETE' });
      for (const upstream_task_id of selected) if (!existing.some(edge => edge.upstream_task_id === upstream_task_id)) await request('/dependencies', { method: 'POST', body: JSON.stringify({ upstream_task_id, downstream_task_id: saved.id }) });
  ...
  return <main className="app">
    ...
    {modal && <div className="scrim" onClick={() => setModal(null)}><form id="task-form" className="modal" onSubmit={save} onClick={event => event.stopPropagation()}>
      ...
      <label>Prerequisites<select name="dependencies" multiple size="4">{tasks.filter(task => task.id !== modal.id).map(task => <option key={task.id} value={task.id} selected={modal.dependencies?.includes(task.id)}>{task.title}</option>)}</select><small>Use Ctrl/Cmd to select multiple tasks.</small></label>
