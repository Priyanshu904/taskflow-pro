...
- **Duplicate titles:** Two tasks with the same title were accepted with distinct IDs (16 and 17). The storage model identifies tasks by ID, but the suggestion approval UI resolves a suggested title to the first matching task. The live AI check did not use duplicate titles, so how the model ranks or emits duplicate title candidates remains unverified.
